...
//Write a register to MPU6500  标记
uint8_t MPU6500_Write_Reg(uint8_t const reg, uint8_t const data)
{
  static uint8_t MPU_Rx, MPU_Tx;
  
  MPU6500_NSS_Low();  //HAL_GPIO_WritePin(GPIOF, GPIO_PIN_6, GPIO_PIN_RESET)
  
  MPU_Tx = reg&0x7f;
  HAL_SPI_TransmitReceive(&hspi5, &MPU_Tx, &MPU_Rx, 1, 55);           //发送接收统一函数 发送一个数据的同时负责接收一个数据
  MPU_Tx = data;
  HAL_SPI_TransmitReceive(&hspi5, &MPU_Tx, &MPU_Rx, 1, 55);
  
  MPU6500_NSS_High();  //HAL_GPIO_WritePin(GPIOF, GPIO_PIN_6, GPIO_PIN_SET)
  return 0;
}
...
//Initialize the IST8310   //三轴地磁传感器
...
uint8_t MPU6500_Init(void)
{
  ...
  {
    {MPU6500_PWR_MGMT_1,    0x80},      // Reset Device 电源管理寄存器1 6B 第7位设置为1，复位MPU6500，复位结束后，MPU硬件自动清零该位。
    {MPU6500_PWR_MGMT_1,    0x03},      // Clock Source - Gyro-Z  001-101 自动选择最有效的时钟源
    {MPU6500_PWR_MGMT_2,    0x00},      // Enable Acc & Gyro 电源管理寄存器2 6c 该寄存器第六位有效 分别控制加速度和陀螺仪的x/y/z轴是否开启，设置为全开启，全部设置为0即可
    {MPU6500_CONFIG,        0x02},      // LPF 98Hz  配置寄存器1A  低三位 低通滤波器设置 陀螺仪根据这三位配置进行过滤 带宽92Hz 延迟3.9ms 
    {MPU6500_GYRO_CONFIG,   0x18},      // +-2000dps  陀螺仪配置寄存1B 陀螺仪满量程范围 +_2000°/S
    {MPU6500_ACCEL_CONFIG,  0x10},      // +-8G  加速度配置器传感器1C  正负8g 65536/16=4096LSB/g  例程 正负2g
    {MPU6500_ACCEL_CONFIG_2,0x02},      // enable LowPassFilter  Set Acc LPF  ？？
    {MPU6500_USER_CTRL,     0x20},      // Enable AUX  用户控制寄存器6A
  };
  
  HAL_Delay(100);
  MPU_id = MPU6500_Read_Reg(MPU6500_WHO_AM_I /*0x75*/);  //read id of device,check if MPU6500 or not
